...
SUPPORTAC = {
  ClassName = "SUPPORTAC",
  useSRS = true, -- if true, messages will be sent over SRS using the MISSIONSRS module. If false, messages will be sent as in-game text.
}
...
function SUPPORTAC:Start()
	_msg = string.format("[JTF-1 SUPPORTAC] Start")
	self:T(_msg)

	-- default to not using SRS unless both the server AND the module request it AND MISSIONSRS.Radio.active is true
	self.useSRS = (JTF1.useSRS and self.useSRS) and MISSIONSRS.Radio.active 
	self:I({"[JTF-1 SUPPORTAC] useSRS", self.useSRS})

	for index, mission in ipairs(SUPPORTAC.mission) do -- FOR-DO LOOP
		_msg = string.format("[JTF-1 SUPPORTAC] Start - mission %s", mission.name)
		...
		if missionHomeAirbase then -- CHECK HOME AIRBASE
			_msg = string.format("[JTF-1 SUPPORTAC] start - Mission %s using %s as home base.", mission.name, missionHomeAirbase)
			SUPPORTAC:T(_msg)
			...
			_msg = string.format("[JTF-1 SUPPORTAC] start - Using spawn template from miz for %s.", missionSpawnType)
			SUPPORTAC:T(_msg)
			...
			_msg = string.format("[JTF-1 SUPPORTAC] start - Using spawn template from SUPPORTAC.template for %s.", missionSpawnType)
			SUPPORTAC:T(_msg)
			...
			_msg = string.format("[JTF-1 SUPPORTAC] Start - template for type %s for mission %s is not present in MIZ or as a predefined template!", missionSpawnType, missionSpawnAlias)
			SUPPORTAC:E(_msg)
			else -- CHECK SKIP
			...
			_msg = string.format("[JTF-1 SUPPORTAC] New late activated mission spawn template added for %s", missionSpawnAlias)
			...
			_msg = string.format("[JTF-1 SUPPORTAC] Start - Default Home Airbase for %s not defined! Mission skipped.", missionTheatre)
			SUPPORTAC:E(_msg)

		end -- CHECK HOME AIRBASE

		else -- CHECK MISSION ZONE
		_msg = string.format("[JTF-1 SUPPORTAC] Start - Zone %s not found! Mission skipped.", mission.zone)
		SUPPORTAC:E(_msg)
...
function SUPPORTAC:NewMission(mission, initDelay)
	_msg = string.format("[JTF-1 SUPPORTAC] Create new mission for %s", mission.name)
	SUPPORTAC:T(_msg)
	...
	if mission.category == SUPPORTAC.category.tanker then
		...
		_msg = string.format("[JTF-1 SUPPORTAC] New mission created: %s", newMission:GetName())
		SUPPORTAC:T(_msg)
	elseif mission.category == SUPPORTAC.category.awacs then
		...
		_msg = "[JTF-1 SUPPORTAC] Mission category not defined!"
		SUPPORTAC:E(_msg)
	...
	_msg = string.format("[JTF-1 SUPPORTAC] New late activated group %s spawned.", spawnGroup:GetName())
	...
	function flightGroup:OnAfterSpawned()
		_msg = string.format("[JTF-1 SUPPORTAC] Flightgroup %s activated.", self:GetName())
		SUPPORTAC:T(_msg)
	...
	function flightGroup:OnAfterMissionStart()
		...
		_msg = string.format("[JTF-1 SUPPORTAC] Mission %s for Flightgroup %s, %s has started.", missionName, flightGroupName, flightGroupCallSign) -- self:GetCallsignName(true)
		SUPPORTAC:T(_msg)
		...
		_msg = string.format("[JTF-1 SUPPORTAC] Flightgroup %s is RTB.", flightGroupName)
		SUPPORTAC:T(_msg)
		end
		...
		_msg = string.format("[JTF-1 SUPPORTAC] newMission OnAfterDone - Mission %s for Flightgroup %s is done.", missionName, flightGroupName)
		SUPPORTAC:T(_msg)
		...
		if flightGroup:IsAlive() and missionDespawn then
			_msg = string.format("[JTF-1 SUPPORTAC] newMission OnAfterDone - Flightgroup %s will be despawned after %d seconds.", flightGroupName, despawnDelay)
			SUPPORTAC:T(_msg)
...
function SUPPORTAC:SendMessage(msgText, msgFreq)
	local _msg = string.format("[JTF-1 SUPPORTAC] SendMessage: %s", msgText)
	SUPPORTAC:T(_msg)
	if useSRS then
