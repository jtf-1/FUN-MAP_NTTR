...
MISSIONSRS = {
  srsPath = "C:/Program Files/DCS-SimpleRadio-Standalone", -- default path to SRS install directory if setting file is not avaialable "C:/Program Files/DCS-SimpleRadio-Standalone"
...
function MISSIONSRS:Start()
  ...
  -- self.srsPath = JTF1.srsPath or self.default.srsPath
  -- self.srsPort = JTF1.srsPort or self.default.srsPort
  -- self.freqs = self.data.freqs or self.default.freqs
  -- self.modulations = self.modulations or self.default.modulations
  -- self.gender = self.gender or self.default.gender
  -- self.culture = self.culture or self.default.culture
  -- self.name = self.name or self.default.name
  self:AddRadio()
...
function MISSIONSRS:SendRadio(msgText, msgFreqs, msgModulations)
  ...
  if msgText == (nil or "") then 
    msgText = self.default.msg
  end
  local text = msgText
  local tempFreqs = msgFreqs or self.default.freqs
  local tempModulations = msgModulations or self.default.modulations
